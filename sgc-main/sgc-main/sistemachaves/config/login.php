...
class Sessao {
    ...
    function logarUser() {
        ...
        try {
            //Prepara uma declaração para validar no banco
            $queryValidar = "select id_cliente, matricula, nome, senha from cliente where matricula = :matricula";
                    ...
                    if ($stmt->rowCount() == 1) {
                        if ($linha = $stmt->fetch()) {
                            ...
                            if (password_verify($this->senha, $senha_hash)) {
                                ...
                                $_SESSION["nomeUser"] = $nome_cliente;
                               

                                //Redireciona-o para a página de gerenciamento de chaves
                                header("location: ../Funcionario/HomeUser.php");
                            }
                            else {
                                $_SESSION["nao_autenticado"] = true;
                                header("location: ../Funcionario/index.php");
